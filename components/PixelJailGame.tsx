...
export default function PixelJailGame() {
  ...
  const act = (type: "feed" | "bribe" | "escape") => {
    ...
    const tk = todayKey();
    let next = { ...stats, lastTick: Date.now() };
  ...
  if (!stats) return <div>Loading inmate file…</div>;

  return (
    <div style={{ border: "1px solid #eee", borderRadius: 12, padding: 16, maxWidth: 520 }}>
      <canvas ref={canvasRef} width={420} height={260} style={{ width: "100%", borderRadius: 12, border: "1px solid #ddd" }} />
      <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
        <button onClick={() => act("feed")}>FEED</button>
        <button onClick={() => act("bribe")}>BRIBE</button>
        <button onClick={() => act("escape")}>ESCAPE</button>
      </div>
    </div>
  );
}
